...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score', 'qr_code', 'invite_code', 'lock_score'];
     ...
     * @return boolean
     */
    public function register($bio, $cnName, $enName, $birthday, $companyName, $inviteCode, $username, $password, $email = '', $mobile = '', $extend = [])
    {
        // 检测用户名、昵称、邮箱、手机号是否存在
...
//        }
        ...
        //邀请码存在解析user_id绑定推广人
        if($inviteCode){
            $params['spread_uid'] = decode_user_id_by_code($inviteCode);
        }
        ...
        try {
            ...
            //绑定个人唯一邀请码
            $this->_user->save(['invite_code' => get_code_by_user_id($user->id)]);
